...
Автозагрузка **не настраивается инсталлятором** ([installer.iss:49-52](../installer.iss)) —
её включает сам пользователь через чекбокс в меню трея. Так путь к exe всегда берётся из
места, откуда программа реально запущена.
